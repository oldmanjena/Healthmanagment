...
namespace Healthmanagment.Anzeigen
{
    /// <summary>
    /// Interaktionslogik f?r GridAnzeigen.xaml
    /// </summary>
    public partial class GridAnzeigen : Window
    {
        ...
        private readonly string a = "SELECT DATEPART(Day, Datum) AS Tag, " +
                                     ...
                                     "FROM Training " +
                                     "WHERE YEAR(Datum) = YEAR(GETDATE()) AND MONTH(Datum) = MONTH(GETDATE()) " +  // Filter f?r das aktuelle Jahr
                                     "GROUP BY DATEPART(Day, Datum)" +
                                     "ORDER BY Tag DESC;";
        private readonly string b = "SELECT DATEPART(ISO_WEEK, Datum) AS Woche, CAST(AVG(Gewicht) AS DECIMAL(10, 2)) AS Diffenenz, CAST(AVG(K?rperfett) AS DECIMAL(10, 2)) AS K?rperfett FROM Fitdays WHERE YEAR(Datum) = YEAR(GETDATE())  GROUP BY DATEPART(ISO_WEEK, Datum) ORDER BY Woche";
